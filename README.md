...
Each task has just one or two dependencies (_except for image optimization_), so I recommend deleting the ones you don't need before running `npm install`. Learn more in [the documentation](#documentation) below.
...
(_Yea, that's kind of lot, isn't it?_)
